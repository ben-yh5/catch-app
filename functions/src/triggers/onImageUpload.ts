 ...
 */
export const onImageUpload = functions.runWith({ memory: '1GB' }).storage.object().onFinalize(async (object) => {
    ...
    try {
        ...
        if (matchDoc) {
            ...
            functions.logger.log(`Updated post ${matchDoc.id} with new URLs`)
        } else {
            functions.logger.warn(`Could not find post for image ${fileName}`)
        }
