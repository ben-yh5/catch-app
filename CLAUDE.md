...
| **Camera & Media** | `react-native-vision-camera` v5 (+ `react-native-nitro-modules`, `react-native-nitro-image`), `expo-image`, `expo-image-picker`, `expo-image-manipulator` |
...
| **Device APIs** | `expo-sensors`, `expo-haptics`, `expo-device`, `expo-file-system`, `expo-linking`, `expo-web-browser`, `expo-network` |
...
- **No reaction layer — deliberately** (Sept 2026): a person-to-person "response to a post" (the postmark, a dated franking mark notifying the author) was designed, built, and **cut the same day** — verdict: it doesn't say much and is extra infrastructure. Likes and lightweight tap-reactions in any costume are out; the meaningful responses are the real acts (catching, following) and deliberate curation (lists). Don't re-pitch reaction features.
- **Catch counts are never displayed on content** (batch 5 minimal cut, Sept 2026): the CatchBadge trophy-count chip was retired from all five surfaces (PostCard, RecommendedPostCard, CompactPostCard, FeaturedPostCard, ThreadModal header; component deleted). `CatchRecencyLine` shows the catch as an *event* instead: on feed/featured cards it's a **scrim chip overlaid bottom-left on the photo** (`variant="chip"`), rendered ONLY when a catch actually happened — never-caught cards carry nothing, so the text block keeps its space; the thread header uses the inline `line` variant with all states ("caught yesterday" caught-pink dot / "not yet caught" / "last caught in June" gray — the last doubles as the lost-place whisper). Derives from `catchCount` + `lastCaughtAt` (`getCatchRecency` in `src/utils/catchRecency.ts`, thresholded by `LOST_PLACE_INACTIVITY_DAYS`). `catchCount`/`lastCaughtAt` stay server-written internal plumbing; counts render only as own-passport stats.
...
- `hotScore`: number, root posts only — server-computed place rank (see Place ranking)
...
| `getRecommendedFeed` | HTTPS Callable | For You list: social + near-you + saved-city candidates, ranked by `hotScore`, finite (no cursor) |
| `backfillHotScores` | HTTPS Callable | Admin-only: (re)writes `hotScore` on all root posts; re-run after changing the formula. `dryRun` optional |
...
- **Platform branching**: Camera is native-only (`react-native-vision-camera`); use `Alert.alert()` on native, `window.alert()` on web
...
- **Place ranking (`hotScore`)**: one score per root post, Reddit-"hot" form `log10(1 + catchCount) + lastActivitySec / HOT_SCORE_PERIOD_SECONDS` (1 week — `functions/src/lib/hotScore.ts`). Time lives inside the score, so it never needs re-decaying: no batch job. Written in the counter transactions of `onPostCreated`/`onPostDeleted` (and on thread promotion); rules block clients from seeding it. Every "best places" surface sorts by it: Explore Trending shelf, MapScreen Trending sort, `getPostsInArea` when a viewport exceeds `MAX_AREA_RESULTS` (originals filtered *before* the cap), and `getRecommendedFeed`. For You = three candidate sources (followed users — a friend's catch surfaces the root with "@x caught this"; roots within 25km of the client's coarse last-known location, never prompted; top places in the cities of recent saves), minus own/already-caught/blocked, sorted by `hotScore`, max 2 consecutive per author, 50 cards, no pagination. Deliberately simple — see `docs/RECOMMENDATION_SYSTEM.md` for what was deferred.
...
- **Offline outbox**: shots/catches submitted offline are queued (photo copied to `documentDirectory`, record in AsyncStorage) and flushed on reconnect/foreground — `src/services/outbox.ts`, `OutboxContext`, `OutboxBanner` (Explore). Catches re-run `validateCatch` (and get their permit) at flush time with the stored shutter coords; the judge and angle checks deliberately don't re-run. Permanent flush failures surface as failed rows with Retry/Discard — never silently dropped. Design doc: `docs/OFFLINE_OUTBOX.md`.
- **Camera (VisionCamera v5)**: `UnifiedCameraView` runs on `react-native-vision-camera` v5 (Nitro; migrated from expo-camera Sept 2026 — its Android zoom clamped to ≥1× and bridged every pinch frame through JS). Pinch-to-zoom and tap-to-focus are the NATIVE gestures (`enableNativeZoomGesture`), which conflicts with the `zoom` prop — never set both. The back device requests the virtual UW+W+T camera so zoom crosses below 1× into real ultrawide; zoom pills (0.5×/1×/2×) work in display units via `neutralRaw = controller.zoom / controller.displayableZoomFactor` (iOS virtual devices open at raw 1 = ultrawide — `onStarted` normalizes to 1×), and a light poll keeps the highlighted pill tracking native pinch (no JS zoom events exist). Capture is `photoOutput.capturePhotoToFile()`. VisionCamera has no Expo config plugin: the CAMERA permission + `NSCameraUsageDescription` live directly in `app.config.js`. Permission hooks: `useCameraPermission()` (`hasPermission`/`canRequestPermission` — request only when `canRequestPermission`, else send to Settings).
...
See `docs/TODO.md` for the prioritized work list. All project docs live in `docs/` — create new
design/reference docs there, not in the project root (root keeps only `CLAUDE.md` and `README.md`).
