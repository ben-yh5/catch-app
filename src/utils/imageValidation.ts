 ...
 */
export const checkBrightness = async (
    uri: string,
    threshold: number = 30
): Promise<boolean> => {
    try {
        ...
        console.log(
            `[ImageValidation] Brightness: ${avgBrightness.toFixed(2)} (Threshold: ${threshold})`
        )
...
export const checkBlur = async (
    ...
    try {
        ...
        console.log(
            `[ImageValidation] Blur Variance: ${variance.toFixed(2)} (Threshold: ${threshold})`
        )
